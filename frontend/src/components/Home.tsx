...
export const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="border-b border-slate-900 bg-(--color-light)">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-4">
          <div>
           ...
           <Button  variant="dark" className='border bg-(--color-text) ' onClick={logout}>
      ...
      <div className="mx-auto flex w-full max-w-6xl gap-6 px-6 py-8">
        <aside className="hidden w-64 shrink-0 flex-col gap-6 md:flex">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-sm font-semibold text-slate-900">{fullName || 'HRMS User'}</p>
            <p className="text-xs text-slate-500">{email || 'user@company.com'}</p>
