...
import { ExpenseSubmitForm, type ExpenseFormValues } from '../../components/travel/ExpenseSubmitForm'
import { ExpenseListItem, type Expense } from '../../components/travel/ExpenseListItem'
import { DraftActions, type DraftEditValues } from '../../components/travel/DraftActions'
import { ExpenseFiltersPanel } from '../../components/travel/ExpenseFiltersPanel'
...
export const ExpensesPage = () => {
  ...
  const handleStartEditDraft = (expenseId: number) => {
    const expense = list?.find((item) => item.expenseId === expenseId)
    if (!expense) return
  ...
  const handleUpdateDraftField = (expenseId: number, field: keyof DraftEditValues, value: string | number) => {
    setDraftEdits((prev) => ({
      ...prev,
      [expenseId]: {
        ...prev[expenseId],
  ...
  const handleSaveDraft = async (expenseId: number) => {
    ...
    if (!payload.categoryId || !payload.amount || !payload.currency || !payload.expenseDate) {
  ...
  const handleReview = async (expenseId: number, formValues: { remarks: string; status: 'Approved' | 'Rejected' }) => {
    try {
  ...
  return (
    <section className="space-y-4">
      <Header title="Expenses" description="Manage your expense submissions and track approvals." />
      ...
      {isEmployee ? (
        <Card>
          <ExpenseSubmitForm
            ...
            isSubmitting={createDraft.isPending || uploadProof.isPending || submitExpense.isPending}
            ...
            >
              ...
              {isEmployee && expense.status === 'Draft' ? (
                <DraftActions
                  ...
                  onFieldChange={(field, value) => handleUpdateDraftField(expense.expenseId, field, value)}
                  onSaveDraft={() => handleSaveDraft(expense.expenseId)}
                  onCancelEdit={() => setEditingDraftId(null)}
                  onDeleteDraft={() => handleDeleteDraftExpense(expense.expenseId)}
                  onUploadProof={() => handleUploadProofOnly(expense.expenseId)}
                  onDeleteProof={(proofId) => handleDeleteProofFile(expense.expenseId, proofId)}
                  onFilePicked={(file) => setDraftFiles((prev) => ({ ...prev, [expense.expenseId]: file }))}
