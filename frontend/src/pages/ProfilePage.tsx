...
import { useUploadProfileAvatar, useUserProfile, useUpdateUserProfile } from '../hooks/useProfile'
...
export const ProfilePage = () => {
  ...
  return (
    <section className="space-y-6">
      <Header title="Profile" description="Update your personal and work details." />
      ...
      {profileQuery.isError ? (
        <Card>
          <p className="text-sm text-red-600">Unable to load profile right now.</p>
        </Card>
      ) : null}
      ...
      {profileQuery.data ? (
        <Card>
          <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <Input
                ...
                type="file"
                accept="image/*"
                onChange={(event) => setAvatarFile(event.target.files?.item(0) ?? null)}
              />
              <Button type="button" disabled={!avatarFile || uploadAvatar.isPending} onClick={onUploadAvatar}>
                {uploadAvatar.isPending ? 'Uploading...' : 'Upload avatar'}
              </Button>
            </div>
          </div>
          {avatarMessage ? <p className="mb-4 text-sm text-emerald-600">{avatarMessage}</p> : null}
          <form className="grid gap-4 md:grid-cols-2" onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <Input label="Role" value={profileQuery.data.role ?? '—'} disabled />
            ...
            <Input
              ...
              disabled
            />
            <Input label="Manager" value={profileQuery.data.manager ?? '—'} disabled />
